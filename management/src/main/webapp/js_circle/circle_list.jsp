        ...
        $(function(){
            $('.del').click(function () {
                ...
                if(confirm("你确定要删除"+id+"的信息吗？")){
                    $.ajax('${pageContext.request.contextPath}/circle/ajaxDelAdm?circleId='+id,{
                        type:'POST',
                        success:function (data) {
                            if(data && data.success) {
                                alert('删除成功');
                                window.location.href='${pageContext.request.contextPath}/circle/listByPage';
                            }else{
                                alert('删除失败');
                            }
                        }
    ...
    <script>
        $(function(){
            $("#toPageBtn").click(function () {
                ...
                window.location.href = "${pageContext.request.contextPath}/circle/listByPage?pageNumStr="+pageNum;
            })
        })
    </script>
</head>
...
<div style="float: left;text-align: center;margin-left: 50px;margin-top: 50px">
    <span style="font-size: 40px">帖子管理-----原贴</span>
...
<div  class="demoTable"  style="height: 20px;float:right;margin-top: 120px;margin-right: 100px">
    ...
    <form action="${pageContext.request.contextPath}/circle/queryByKeyWord" method="post">
...
<div  style="float:left;padding-top: 80px; padding-left:50px;width: 100%;">
    ...
    <c:choose>
        ...
        <c:otherwise>
            <a href="${pageContext.request.contextPath}/circle/listByPage?pageNumStr=${pageNum-1}" class="pagea"  style="color: black">上一页</a>
        </c:otherwise>
    </c:choose>
    &nbsp;&nbsp;
    <c:choose>
        ...
        <c:otherwise>
            <a href="${pageContext.request.contextPath}/circle/listByPage?pageNumStr=${pageNum+1}" class="pagea"  style="color: black">下一页</a>
        </c:otherwise>
    </c:choose>
...
<div style="float:left; width: 100%;height: 100%;margin-top: 10px;" >
    ...
    <table width="100%" border="1"  cellspacing="0" cellpadding="0">
        ...
        <c:forEach items="${circleList}" var="circle" varStatus="stats">

            <tr>

                <td width="5%">${stats.count}</td>
                <td width="8%">${circle.circleId}</td>

                <td width="10%">${circle.circleTheme}</td>

                <td width="30%">${circle.circleContent}</td>

                <td width="10%">

                    <%
                        Circle circle = (Circle) pageContext.getAttribute("circle");
                        User user  = circle.getUser();
                ...
                <td width="10%"><fmt:formatDate value="${circle.circlePublicTime}"/></td>
                    <%--循环显示图片--%>
                    <%--  <td width="20%">
                ...
                <td width="10%">
                   <%-- <a href="${pageContext.request.contextPath}/circle/toEdit?id=${circle.circleId}" >
                        <BUTTON style="background: #d98c1d;color: black">查看原帖</BUTTON>
                    </a>--%>
                    <a href="javascript:void(0)" class="del" idVal="${circle.circleId}">
